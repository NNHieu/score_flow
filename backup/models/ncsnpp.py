...
def get_time_embeding(embedding_type, 
                 sde: str, 
                 continuous: bool, 
                 nf, 
                 fourier_scale, 
  ...
  if embedding_type == 'fourier':
    ...
    assert continuous, "Fourier features are only used for continuous training."
    if sde.lower() == 'vesde':
...
class Unet(nn.Module):
  ...
  PreDownBlock: nn.Module
  Downsample: nn.Module
  PreUpBlock: nn.Module
  Upsample: nn.Module
  CenterBlock: nn.Module
  Out: nn.Module

  @nn.compact
  def __call__(self, x, temb, train) -> Any:
    ...
    for i_level in range(num_resolutions):
      ...
      h, save_hs = self.PreDownBlock(self.nf * self.ch_mult[i_level])(h, temb, train)
      hs[i_level].extend(save_hs)
      if i_level != num_resolutions - 1:
        h, input_pyramid = self.Downsample()(h, temb, input_pyramid, train)
        hs.append([h])
    
    h = self.CenterBlock()(h, temb, train)

    # Upsampling block
    pyramid = None
    for i_level in reversed(range(num_resolutions)):
      h = self.PreUpBlock(self.nf * self.ch_mult[i_level])(h, temb, hs.pop(), train)
      h, pyramid = self.Upsample()(h, temb, pyramid, train)
    assert not hs
    return self.Out()(h)
...
class PreDownBlock(nn.Module):
  ...
  @nn.compact
  def __call__(self, h, temb, train) -> Any:
    # Residual blocks for this resolution
    save_hs = []
    for _ in range(self.num_res_blocks):
      h = self.ResnetBlock(out_ch=self.num_channels)(h, temb, train)
...
class Down(nn.Module):
  ...
  PyramidDown: nn.Module
  fir: Any
  fir_kernel: Any
  resamp_with_conv: Any
  ResnetBlock: nn.Module
  Combine: nn.Module
  ...
  def __call__(self, h, temb, train) -> Any:
    if self.resblock_type == 'ddpm':
      ...
      h = self.ResnetBlock(down=True)(h, temb, train)

    return h

class ProgressiveInput(nn.Module):
  def __call__(self, *args, **kwargs) -> Any:
    if self.progressive_input == 'input_skip':
      input_pyramid = self.PyramidDown()(input_pyramid)
      h = self.Combine()(input_pyramid, h)
    elif self.progressive_input == 'residual':
      input_pyramid = self.PyramidDown(out_ch=h.shape[-1])(input_pyramid)
      input_pyramid = input_pyramid + h
      if self.skip_rescale:
        input_pyramid = input_pyramid / np.sqrt(2.)
      h = input_pyramid
...
class PreUpBlock(nn.Module):
  ...
  @nn.compact
  def __call__(self, h, temb, save_hs, train) -> Any:
    # Residual blocks for this resolution
    for i_block in range(self.num_res_blocks + 1):
      h = self.ResnetBlock(out_ch=self.num_channels)(jnp.concatenate([h, save_hs.pop()], axis=-1),
                                                    temb,
                                                    train)
    if h.shape[1] in self.attn_resolutions:
      h = self.AttnBlock()(h)
...
class OutputSkipUp(nn.Module):
  act: Callable

  @nn.compact
  def __call__(self, h, temb, pyramid, train) -> Any:
    act = self.act
    if i_level == num_resolutions - 1:
      pyramid = conv3x3(
      ...
      pyramid = self.PyramidUpsample()(pyramid)
...
class NCSNpp(nn.Module):
  ...
  nonlinearity: str
  num_res_blocks: int
  TimeEmbLayer: nn.Module 
  AttnBlock: Callable
  Upsample: Callable
  PyramidUpsample: Callable
  ...
  def __call__(self, x, time_cond, train=True):
    ...
    temb = self.TimeEmbLayer()(time_cond)
      ...
      if progressive != 'none':
        if i_level == num_resolutions - 1:
          ...
          if progressive == 'output_skip':
            ...
            pyramid = self.PyramidUpsample(out_ch=h.shape[-1])(pyramid)
            if skip_rescale:

...
def get_model_fn(model, train=False) -> Callable[[jnp.ndarray, jnp.ndarray, Optional[Any]], Any]:
...
def get_score_fn(sde, model_apply_fn, continuous=False, return_state=False) -> Callable[..., Any]:
  ...
  if isinstance(sde, sde_lib.VPSDE) or isinstance(sde, sde_lib.subVPSDE):
    def score_fn(params, states, x, t, rng=None):
      ...
      if continuous or isinstance(sde, sde_lib.subVPSDE):
        ...
        labels = t * 999
        model, state = model_apply_fn(params, states , x, labels, rng)
    ...
    def score_fn(variables, x, t, rng=None):
      if sde.linear is False:
        ...
        score, state = model_apply_fn(variables, x, labels, rng)
      else:
        assert continuous
        labels = t * 999
        model, state = model_apply_fn(variables, x, labels, rng)
        std = sde.marginal_prob(jnp.zeros_like(x), t)[1]
        score = batch_mul(-model, 1. / std)

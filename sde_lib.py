...
def get_sde(sde_name, model_config):
  if sde_name.lower() == 'vpsde':
    sde = VPSDE(beta_min=model_config.beta_min, beta_max=model_config.beta_max, N=model_config.num_scales)
  elif sde_name.lower() == 'subvpsde':
    sde = subVPSDE(beta_min=model_config.beta_min, beta_max=model_config.beta_max, N=model_config.num_scales)
  elif sde_name.lower() == 'vesde':
    sde = VESDE(sigma_min=model_config.sigma_min, sigma_max=model_config.sigma_max, N=model_config.num_scales)
  elif sde_name.lower() == 'linearvesde':
    sde = VESDE(sigma_min=model_config.sigma_min, sigma_max=model_config.sigma_max, N=model_config.num_scales,
                        linear=True)
  else:

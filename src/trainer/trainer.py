...
class Trainer(object):
  ...
  def log(self, name, val, step_idx):
    log.info(f"Training log at step {step_idx} | {name}: {val}")

  def setup_logger(self):
    self.logger = utils.SimpleLogger(self.tb_dir)

  def save(self, pstate, rng, step, preemtion=False):
  ...
  def fit(self, rng, state, datamodule: datasets.DataModule):
    self._call_and_handle_interrupt(self._fit, rng, state, datamodule)

  def _fit(self, rng, state, datamodule: datasets.DataModule):

...
def get_sampling_fn(config, sde, score_apply_fn, shape, inverse_scaler, eps):
...
def get_ode_sampler(sde, score_apply_fn, shape, inverse_scaler,
  ...
  def denoise_update_fn(rng, state, x):
    ...
    score_fn = functools.partial(score_apply_fn, state)
